...
public class MovimientoCamionVidrioBehaviour : MonoBehaviour {
	...
	void Update ()
    {
        ...
        else if (estadoRecogedor==EN_RECORRIDO)
        {
            ...
            {
                esteCamion.recogerVidrio(casaAVisitar);
                for (int i = 0; i < esteCamion.LoRecogidoVidrio ; i++)
                {
                    Destroy(casaAVisitar.darVidrioAcumulado()[i].darGameobject());
                    casaAVisitar.darVidrioAcumulado()[i] = null;
                     uniqueCiudad.NumBasuraSinRecoger--;
                }
                casaAVisitar.darVidrioAcumulado().RemoveAll((o) => o == null);
        ...
        {
            int ganado = (int)(esteCamion.Llenado * 1.25f);
            estadoRecogedor = STAND_BY;
            miFabrica.CantidadRecogida += esteCamion.Llenado;
            uniqueCiudad.NumVidrioReciclado += esteCamion.Llenado;
