...
public class MovimientoCamionPlasticoBehaviour : MonoBehaviour {
	...
	void Update ()
    {
        ...
        else if (estadoRecogedor == EN_RECORRIDO)
        {
            ...
            {
                esteCamion.recogerPlastico(casaAVisitar);
                for (int i = 0; i < esteCamion.LoRecogidoPlastico; i++)
                {
                    Destroy(casaAVisitar.darPlasticoAcumulado()[i].darGameobject());
                    casaAVisitar.darPlasticoAcumulado()[i] = null;
                    uniqueCiudad.NumBasuraSinRecoger--;
                }
                casaAVisitar.darPlasticoAcumulado().RemoveAll((o) => o == null);
        ...
        {
            int ganado = (int)(esteCamion.Llenado * 1.75f);
            estadoRecogedor = STAND_BY;
            miFabrica.CantidadRecogida += esteCamion.Llenado;
            uniqueCiudad.NumPlasticoReciclado += esteCamion.Llenado;
